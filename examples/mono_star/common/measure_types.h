...
namespace star
{
    ...
    struct SingleMeasureBuffer
    {
        ...
        operator SingleMeasureMap() const
        { // Auto-transfer
    ...
    struct MeasureBuffer : FrameBuffer
    {
        ...
        Measure4Solver GenerateMeasure4Solver(const unsigned num_cam) const
        {
            ...
            {
                measure4solver.vertex_confid_map[cam_idx] = buffer[cam_idx].vertex_confid.texture;
                measure4solver.normal_radius_map[cam_idx] = buffer[cam_idx].normal_radius.texture;
                measure4solver.index_map[cam_idx] = buffer[cam_idx].index.texture;
        ...
        Measure4Fusion GenerateMeasure4Fusion(const unsigned num_cam) const
        {
            ...
            for (auto cam_idx = 0; cam_idx < num_cam; ++cam_idx)
            {
                measure4fusion.vertex_confid_map[cam_idx] = buffer[cam_idx].vertex_confid.texture;
                measure4fusion.normal_radius_map[cam_idx] = buffer[cam_idx].normal_radius.texture;
                measure4fusion.color_time_map[cam_idx] = buffer[cam_idx].color_time.texture;
                measure4fusion.index_map[cam_idx] = buffer[cam_idx].index.texture;
            }
        ...
        Measure4GeometryRemoval GenerateMeasure4GeometryRemoval(const unsigned num_cam) const
        {
            ...
            for (auto cam_idx = 0; cam_idx < num_cam; ++cam_idx)
            {
                measure4geometry_removal.depth4removal_map[cam_idx] = buffer[cam_idx].depth4removal.texture;
            }
