...
inline void setPoint(float x, float y, float z, PointCloud3f_Pointer &point_cloud, int index, float scale = 1.f)
{
#ifdef WITH_PCL
    pcl::PointXYZ point;
    point.x = x * scale;
...
inline void setPointRGB(float x, float y, float z, unsigned char r, unsigned char g, unsigned char b, PointCloud3fRGB_Pointer &point_cloud, int index,
                        float scale = 1.f)
{
#ifdef WITH_PCL
...
inline void setNormal(float x, float y, float z, PointCloudNormal_Pointer &normal_cloud, int index)
{
#ifdef WITH_PCL
    pcl::Normal normal;
    normal.normal_x = x;
...
inline void setPointCloudSize(PointCloud3f_Pointer &point_cloud, int size)
{
#ifdef WITH_CILANTRO
...
inline void setPointCloudRGBSize(PointCloud3fRGB_Pointer &point_cloud, int size)
{
#ifdef WITH_CILANTRO
    point_cloud->points.resize(PointCloud3f::Dimension, size);
...
inline void setNormalCloudSize(PointCloudNormal_Pointer &point_cloud, int size)
{
#ifdef WITH_CILANTRO
...
inline PointCloud3f_Pointer transformPointCloud(const PointCloud3f_Pointer &input, Eigen::Matrix4f transformation)
{
    ...
    for (auto iPoint = 0; iPoint < input->points.size(); iPoint++)
    {
        ...
        transed_point.x = transed_point4(0);
        transed_point.y = transed_point4(1);
        transed_point.z = transed_point4(2);
...
inline PointCloud3fRGB_Pointer transformPointCloudRGB(const PointCloud3fRGB_Pointer &input, Eigen::Matrix4f transformation)
{
