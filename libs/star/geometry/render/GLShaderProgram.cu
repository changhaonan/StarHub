...
star::GLShaderProgram::GLShaderProgram() : m_vertex_shader(0),
										   m_fragment_shader(0),
										   m_geometry_shader(0),
										   m_program_id(0)
{
...
star::GLShaderProgram::~GLShaderProgram()
{
 ...
 */
void star::GLShaderProgram::Compile(const std::string &vertex_path, const std::string &fragment_path)
{
...
void star::GLShaderProgram::Compile(const char *vertex_shader, const char *fragment_shader, const char *geometry_shader)
{
...
void star::GLShaderProgram::checkShaderCompilerError(GLuint shader, const std::string &type)
{
	int success;
	char infoLog[1024];
 ...
 */
void star::GLShaderProgram::SetUniformMatrix(const char *name, const star::Matrix4f &value)
{
	auto uniform_loc = glGetUniformLocation(ProgramID(), name);
	glUniformMatrix4fv(uniform_loc, 1, GL_FALSE, value.data());
}

void star::GLShaderProgram::SetUniformVector(const char *name, const star::Vector4f &vec)
{
	auto uniform_loc = glGetUniformLocation(ProgramID(), name);
	glUniform4f(uniform_loc, vec[0], vec[1], vec[2], vec[3]);
}

void star::GLShaderProgram::SetUniformVector(const char *name, const float4 &vec)
{
	auto uniform_loc = glGetUniformLocation(ProgramID(), name);
	glUniform4f(uniform_loc, vec.x, vec.y, vec.z, vec.w);
}

void star::GLShaderProgram::SetUniformVector(const char *name, const float3 &vec)
{
	auto uniform_loc = glGetUniformLocation(ProgramID(), name);
	glUniform3f(uniform_loc, vec.x, vec.y, vec.z);
}

void star::GLShaderProgram::SetUniformVector(const char *name, const float2 &vec)
{
	auto uniform_loc = glGetUniformLocation(ProgramID(), name);
	glUniform2f(uniform_loc, vec.x, vec.y);
}

void star::GLShaderProgram::SetUniformFloat(const char *name, const float value)
{
	auto uniform_loc = glGetUniformLocation(ProgramID(), name);
