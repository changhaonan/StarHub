...
#pragma unroll
	...
	__global__ void RemoveNodeOutTrackKernel(
		...
		if (counter_node_outtrack[idx] >= counter_node_outtrack_threshold &&
			node_status[idx].y != DELETED_NODE_STATUS) {  // Set new false, old don't change
...
#pragma unroll
	...
	__global__
	void NodeSemanticProbVoteKernel(
...
#pragma unroll
	...
	__global__
	void NodeSemanticProbAverageKernel(
		...
		const auto idx = threadIdx.x + blockDim.x * blockIdx.x;
		if (idx >= (node_size - num_node_offset)) return;
...
#pragma unroll
		for (auto i = 0; i < d_max_num_semantic; ++i) {
			auto node_semantic_prob_idx_i = unsigned char(node_patch_semantic_prob_vote_idx[i] / semantic_prob_sum * 255.f);
			// Replace with new one (Average?)
			node_semantic_prob_idx[i] = node_semantic_prob_idx_i;
		}
