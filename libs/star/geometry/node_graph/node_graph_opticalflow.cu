...
namespace star { namespace device {
	...
	__global__  
	void AtomicSumIdealNodeMotionFromOpticalFlowKernel(
		...
		if (idx_prev > image_width || idy_prev >= image_height) return;
		...
		if (index_prev == 0xFFFFFFFF || rgbd.w == 0.f) return;
		...
		if (fabs(est_motion_cam.z) > d_opticalflow_match_thresh) return;
	...
	void AverageNodeMotionKernel(
		...
		if (_node_motion_pred.w == 0.f) return;
	...
	// Ideal: use pefect camera points to compute motion
	__global__
	void IdealSurfelMotionFromOpticalFlowKernel(
		cudaTextureObject_t vertex_confid_map_prev,
		cudaTextureObject_t vertex_confid_map,
		cudaTextureObject_t rgbd_map_prev,
		cudaTextureObject_t rgbd_map,
