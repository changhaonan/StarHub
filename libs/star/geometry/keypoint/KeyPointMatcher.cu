    ...
    cudaStream_t stream)
{
    ...
    for (auto j = 0; j < knn_matches.size(); j++)
    {
        if (knn_matches[j].size() >= 2)
        {
            ...
            if (knn_matches[j][0].distance < ratio_thresh * knn_matches[j][1].distance && kp_dist < dist_thresh && kp_dist > 1e-6f)
            {
                good_matches_int2.push_back(make_int2(knn_matches[j][0].queryIdx, knn_matches[j][0].trainIdx));
            }
        }
    ...
    cudaStream_t stream)
{
    ...
    for (auto j = 0; j < knn_matches.size(); j++)
    {
        if (knn_matches[j].size() >= 2)
        {
            ...
            if (knn_matches[j][0].distance < ratio_thresh * knn_matches[j][1].distance && kp_vertex_dist < vertex_dist_thresh)
            {
                good_matches_int2.push_back(make_int2(knn_matches[j][0].queryIdx, knn_matches[j][0].trainIdx));
            }
        }
    ...
    // Log info
    std::cout << "All matches: " << knn_matches.size() << ", good matches: " << good_matches_int2.size() << "." << std::endl;
