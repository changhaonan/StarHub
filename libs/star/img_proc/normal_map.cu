...
namespace star::device
{
	...
	__host__ __device__ __forceinline__ float compute_radius(
		float depth_value, // [m]
		float normal_z,
		float surfel_radius_scale)
	{
		const float radius = depth_value * (1000 * 1.414f / 570.0f) * surfel_radius_scale;
		...
		const float surfel_radius_scale)
	{
		...
		if (!is_zero_vertex(vertex_center))
		{
			...
			if (counter > (window_size / 2))
			{
				...
				// The radius
				const float radius = compute_radius(vertex_center.z, normal.z, surfel_radius_scale);
...
void star::createNormalRadiusMap(
	cudaTextureObject_t vertex_map,
	const unsigned rows, const unsigned cols,
	cudaSurfaceObject_t normal_radius_map,
	const float surfel_radius_scale,
