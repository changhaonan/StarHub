...
namespace star
{
	...
	__host__ __device__ __forceinline__ DualQuaternion averageDualQuaternion(
		const DualQuaternion *warp_field,
		const unsigned short *knn_patch,
		const float *knn_patch_spatial_weight,
		const float *knn_patch_connect_weight,
		const unsigned patch_size)
	{
		...
		{
			const float spatial_weight = knn_patch_spatial_weight[i];
			const float connect_weight = knn_patch_connect_weight[i];
			const unsigned nn_id = knn_patch[i];
			dq_average += DualNumber(spatial_weight * connect_weight, 0) * warp_field[nn_id];
		}
		return dq_average;
	}

	// Used only for debug
	__host__ __device__ __forceinline__ DualQuaternion averageDualQuaternionDebug(
		const DualQuaternion *warp_field,
		const unsigned short *knn_patch,
		const float *knn_patch_spatial_weight,
		const float *knn_patch_connect_weight,
		const unsigned patch_size,
		bool &zero_averge_error)
	{
		...
		for (auto i = 0; i < patch_size; ++i)
		{
			...
			const unsigned nn_id = knn_patch[i];
			dq_average += DualNumber(spatial_weight * connect_weight, 0) * warp_field[nn_id];
			weight_sum += connect_weight * spatial_weight;
		}
		if (weight_sum == 0.f)
			zero_averge_error = true;
		return dq_average;
	}

	template <unsigned short patch_size>
	__host__ __device__ __forceinline__ DualQuaternion averageDualQuaternion(
		const DualQuaternion *warp_field,
		const ushortX<patch_size> knn,
		const floatX<patch_size> knn_spatial_weight,
		const floatX<patch_size> knn_connect_weight)
	{
		DualQuaternion dq_average;
		dq_average.set_zero();
#pragma unroll
		for (auto i = 0; i < patch_size; ++i)
		{
			const float spatial_weight = knn_spatial_weight[i];
			const float connect_weight = knn_connect_weight[i];
			const unsigned nn_id = knn[i];
			dq_average += DualNumber(spatial_weight * connect_weight, 0) * warp_field[nn_id].normalized();
		}
		return dq_average;
	}
	...
	__host__ __device__ __forceinline__ DualQuaternion averageDualQuaternion(
		const DualQuaternion *warp_field,
		const ushortX<patch_size> knn,
		const floatX<patch_size> knn_weight)
	{
		DualQuaternion dq_average;
		dq_average.set_zero();
#pragma unroll
		for (auto i = 0; i < patch_size; ++i)
		{
			const float weight = knn_weight[i];
			const unsigned nn_id = knn[i];
			dq_average += DualNumber(weight, 0) * warp_field[nn_id].normalized();
		}
		return dq_average;
	}
