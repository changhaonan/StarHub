...
star::GenericFileFetch<max_cam>::GenericFileFetch(const fs::path& data_path, std::string extension, bool force_no_masks) :
		m_mask_buffer_ix(SIZE_MAX), m_use_masks(false)
{
	...
	for (unsigned cam_idx = 0; cam_idx < max_cam; ++cam_idx) {  // Check frame number equivilence
	...
	if (!force_no_masks) {
		if (unexpected_mask_frame_number) {
			...
			if (mask_idx_fit) m_use_masks = true;
		}
	}
}
...
int star::GenericFileFetch<max_cam>::GetFrameNumber(const star::GenericFileFetch<max_cam>::path& filename)
{
	...
	if (!std::regex_search(filename_stem, match_result, digits_regex)) {  // Number not sufficient
