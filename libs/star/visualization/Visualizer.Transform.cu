...
namespace star::device
{
		...
		unsigned char *flatten_validity_indicator)
	{
		...
		if (x_center >= img_cols || y_center >= img_rows)
			return;
...
void star::visualize::DrawValidIndexMap(cudaTextureObject_t index_map, int validity_halfsize)
{
	// Query the map
...
void star::visualize::SaveValidIndexMap(
	cudaTextureObject_t index_map,
	int validity_halfsize,
...
cv::Mat star::visualize::GetValidityMapCV(cudaTextureObject_t index_map, int validity_halfsize)
{
	// Query the size
...
void star::visualize::MarkValidIndexMapValue(
	cudaTextureObject_t index_map,
	int validity_halfsize,
	star::GArray<unsigned char> flatten_validity_indicator)
{
	// Query the size
	unsigned width, height;
	query2DTextureExtent(index_map, width, height);
